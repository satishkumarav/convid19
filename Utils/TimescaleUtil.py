...
def getSpreadColumnNames():
    columns = [ColoumnName.TimestampUTC.value, ColoumnName.NameofState_UT.value, ColoumnName.Totalconfirmed.value, ColoumnName.Death.value, ColoumnName.Cured_Discharged_Migrated.value,
               ColoumnName.TotalConfirmedcases_IndianNational.value, ColoumnName.TotalConfirmedcases_ForeignNational.value, ColoumnName.MortalityRate.value]
    return columns
...
def insert2spread(dfRegion, locationparent="India", locationtype=LocationType.State, usetimestampfromdataframe=False,
                  cleanbeforeload=False,deletetodaysrecordsforparentlocation=False,deletetodaysrecordsforlocation=False,location=""):
